function Main()
{
    ...
    var helicopterBox = new THREE.Object3D();
    var helicopterOnly;
    ...
    option.addEventListener("click", function () {
        ...
        helicopterOnly = heliSelection.spawnSelectedHeli()["helicopterBox"];
        helicopterBox.add(helicopterOnly);
        ...
        helicopterOnly.name = helicopterOnly.uuid
        startFlying();
    });
    var client = io();
    ...
    function startFlying() {
        ...
        client.emit("flying", {
            type: helicopterOnly.rodzaj,
            uuid: helicopterOnly.uuid,
            position: helicopterBox.position,
            rotations: rotations
        })
        setTimeout(startFlying, 10);
    }
    client.on("flying", function (data) {
        ...
        var rotations = data.data.rotations;

        var _heli = scene.getObjectByName(uuid);
        if (_heli == undefined) {
            if (type == "kolor") {
                var _helio = new Helicopter();
                _heli = _helio.SpawnHelicopter();
                _heli.rodzaj = "kolor";
                _heli.uuid = uuid;
                _heli.name = uuid;
                scene.add(_heli);
                extraHeli.push(_helio)
            }
            else if (type == "model") {
                 var _helio = new HeliModel();
                 _heli = _helio.spawnModel();
                 _heli.rodzaj = "model";
                 _heli.uuid = uuid;
                 _heli.name = uuid;
                 scene.add(_heli);
        ...
        _heli.position.x = pos.x;
        _heli.position.y = pos.y;
        _heli.position.z = pos.z;

        _heli.rotation.x = rotations.x;
        _heli.rotation.y = rotations.y;
       
        _heli.rotation.z = rotations.z;
    ...
    function AnimateScene() {
        ...
        try {
            ...
            helicopterBox.rotation.y += Math.PI / 180 * heliRot / 100;
            helicopterOnly.rotation.x = -heliRot / 500;

            helicopterOnly.rotation.z = heliHeight / 500;

            for (var h in extraHeli) {
                extraHeli[h].PropRotation();
            }

            rotations.x = helicopterOnly.rotation.x;
            rotations.y = helicopterBox.rotation.y;
            rotations.z = helicopterOnly.rotation.z;
    ...
    function onKeyPress(e) {
        ...
        if (e.which == 113) //"q"
        {
            ...
            helicopterOnly.rotateX(Math.PI / 180 * 10)
            //helicopterOnly.rotateZ(Math.PI / 180 * 10)
