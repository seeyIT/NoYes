...
function resetKeyboardIlluminate(x, z) {
    try {
    if (playFieldTable[z][x].children[1] != memo.currentElement)
...
function loseTurn(cardName) {
    ...
    var _field = memo.scene.getObjectByName(cardName);
    console.log(_field)
...
function clearBoard() {
    var _children = memo.scene.children;
    for (var child in _children) {
        memo.scene.remove(_children[child]);
    }
}
...
function cardClick(parentObject) {
    ...
    //if (!memo.canMakeMove && parentObject !== firstCard)
    ...
    animationCard(parentObject.name, function () {
        ...
        if (!memo.canMakeMove && parentObject !== memo.firstCard)
            checkMove(memo.firstCard, parentObject);
    });

    client.emit("click", parentObject.name);
}

function animationCard(parentObjectName, callback) {

    var parentObject = memo.scene.getObjectByName(parentObjectName);
...
function checkMove(firstCard, secondCard) {
    ...
    if (firstCard.uuid === secondCard.uuid) {
        //TODO  EMIT SUKCESU
        client.emit("keep_playing");
        client.emit("get_info_bar");

        client.emit("success_exposure", firstCard.name, secondCard.name);
        ...
        //TODO  EMIT FAILA
        client.emit("switch_turn");
        client.emit("get_info_bar");

        client.emit("fail_exposure", firstCard.name, secondCard.name);
...
function successExposure(firstCardName, secondCardName) {
    var firstCard = memo.scene.getObjectByName(firstCardName);
    var secondCard = memo.scene.getObjectByName(secondCardName);
...
function resetStatistics() {
    memo.firstCard = null;
    memo.canMakeMove = false; // ustawiamt to po zmianie na emicie
    memo.currentElement = null;
    memo.isCheckingCards = false;
}
